coil = (function() {
    ...
    //     arrayOfSvgPieces.unshift('<?xml version="1.0" encoding="UTF-8" standalone="no"?>');
    //     arrayOfSvgPieces.unshift('<svg width="' + svgWidth + 'px" height="' + svgHeight + 'px"  viewBox="0 0 ' + svgWidth + ' ' + svgHeight + '" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">');

    //     var svgCode = arrayOfSvgPieces.join("");
    //     return svgCode;
    // }

    function generateSvg(coiledStreets, meterPerPixel, svgHeight, stroke){
        stroke = stroke || '#000000' // '#50E3C2';
        var svgPieces = generateSvgPieces(coiledStreets, meterPerPixel, svgHeight, stroke);
        var svgCode = svgPieces.join('');
        return svgCode;
    }

    function generateSvgPieces(coiledStreets, meterPerPixel, svgHeight, stroke, strokeWidth) {
        stroke = stroke || '#000000' // '#50E3C2';
        var svgPieces = [];
        for (var s = 0; s < coiledStreets.length; s++) {
            var coiledStreet = coiledStreets[s];
            var path = generatePath(coiledStreet, meterPerPixel, stroke, strokeWidth);
            svgPieces.push(path);
        }

        svgPieces.push('</svg>');
        var mainString = '<svg ';
        mainString += 'width="800px" height="' + (svgHeight + 100) + 'px"  ';
    ...
    function generatePath(coiledStreet, meterPerPixel, stroke, strokeWidth) {
        ...
        pathPieces.push('<path id="' + name + '" stroke="' + stroke + '" stroke-width="' + strokeWidth + '" fill="none" moovel_area="' + coiledStreet.tags.area.toFixed(2) + '" moovel_length="' + coiledStreet.tags.length.toFixed(2) + '" d="');
