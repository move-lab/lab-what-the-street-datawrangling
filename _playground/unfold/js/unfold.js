var unfold = (function() {
    ...
    function geoJsonStreetAnimation(originalStreet, coiledStreet, progressUnfold, progressRestitch) {
        ...
        for (var j = 0; j < coiledGeometry.length; j++) {
            ...
            //cursor.geometry.coordinates[0] = roundToDecimals(cursor.geometry.coordinates[0], precisionCoordinates);
            //cursor.geometry.coordinates[1] = roundToDecimals(cursor.geometry.coordinates[1], precisionCoordinates);
    ...
    function subdivideVectorStreet(vectorStreet, maximumLengthInMeter) {
        ...
        for (var i = 0; i < vectors.length; i++) {
            ...
            for (var j = 0; j < numberOfParts; j++) {
                ...
                // Simplify
                newVector.deltaBearing = roundToDecimals(newVector.deltaBearing, 2);
                newVector.distance = roundToDecimals(newVector.distance, 7)
            ...
            for (var p = 0; p < paths.length; p++) {
                ...
                for (var n = 0; n < path.length; n++) {
                    ...
                    if (c == 0 && p == 0 && n == 0) {
                        ...
                        //Simplify
                        //deltaBearing = roundToDecimals(deltaBearing, precisionPixels);
                        //distance = roundToDecimals(deltaBearing, precisionPixels);
